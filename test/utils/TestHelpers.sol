...
library TestHelpers {
    ...
    function appendFacet(
        DesiredFacetsIO.Facet[] memory arr,
        DesiredFacetsIO.Facet memory x
    ) internal pure returns (DesiredFacetsIO.Facet[] memory out) {
    ...
    function removeFacetAt(
        DesiredFacetsIO.Facet[] memory arr,
        uint256 idx
    ) internal pure returns (DesiredFacetsIO.Facet[] memory out) {
        require(idx < arr.length, "TestHelpers: Index out of bounds");
        out = new DesiredFacetsIO.Facet[](arr.length - 1);
        for (uint256 i = 0; i < idx; i++) out[i] = arr[i];
        for (uint256 i = idx + 1; i < arr.length; i++) out[i - 1] = arr[i];
    }
    ...
    function createFacet(
        string memory artifact,
        string[] memory uses
    ) internal pure returns (DesiredFacetsIO.Facet memory facet) {
        return DesiredFacetsIO.Facet({
            artifact: artifact,
    ...
    function createFacetWithNamespace(
        string memory artifact,
    ...
    function appendNamespace(
        StorageConfigIO.NamespaceConfig[] memory a, 
        StorageConfigIO.NamespaceConfig memory x
    ) internal pure returns (StorageConfigIO.NamespaceConfig[] memory b) {
    ...
    function dropByArtifact(DesiredFacetsIO.Facet[] memory a, string memory artifact) internal pure returns (DesiredFacetsIO.Facet[] memory b) {
        uint256 keep = 0; 
    ...
    function setUsesOnly(DesiredFacetsIO.DesiredState memory d, string memory artifact, string memory onlyNs) internal pure {
        for (uint256 i = 0; i < d.facets.length; i++) if (stringEquals(d.facets[i].artifact, artifact)) {
